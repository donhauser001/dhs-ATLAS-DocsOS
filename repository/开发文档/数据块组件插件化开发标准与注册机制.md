# 数据块组件插件化开发标准与注册机制

> 版本: 2.0.0  
> 更新日期: 2026-01-05  
> 状态: ✅ 已实现

## 概述

ATLAS DocsOS 采用**插件化组件架构**，允许开发者通过简单地创建符合规范的组件目录来扩展系统功能。框架会自动扫描并注册组件，无需修改任何框架代码。

### 核心特性

- 🔌 **即插即用** - 新组件只需放入指定目录即可自动注册
- 📦 **模块隔离** - 每个组件独立目录，互不干扰
- 🔧 **统一接口** - 标准化的类型定义和 Props 规范
- 🎨 **双视图分离** - Control（数据块内控件）与 Configurator（配置弹窗）分离

## 目录结构

```
frontend/src/components/visual-editor/ComponentRegistry/
├── components/                    # 组件目录（每个子目录一个组件）
│   ├── select/                   # 下拉选择组件
│   │   ├── config.ts            # 元数据 & 默认值
│   │   ├── Control.tsx          # 数据块内渲染控件
│   │   ├── Configurator.tsx     # 配置弹窗表单
│   │   └── index.ts             # 注册入口（必须）
│   ├── multi-select/
│   ├── rating/
│   ├── number/
│   ├── date/
│   ├── text/
│   ├── textarea/
│   ├── file/
│   ├── files/
│   ├── image/
│   └── images/
├── shared/                       # 共享工具
│   ├── utils.ts                 # 工具函数
│   ├── OptionEditor.tsx         # 选项编辑器（可复用）
│   ├── FallbackControl.tsx      # 降级控件
│   └── index.ts
├── types.ts                      # 类型定义
├── registry.ts                   # 注册中心（自动扫描）
├── ComponentControl.tsx          # 统一控件入口
├── ComponentConfigurator.tsx     # 统一配置器入口
└── index.ts                      # 导出
```

## 快速开始：创建新组件

假设要创建一个 `color-picker`（颜色选择器）组件：

### 1. 创建组件目录

```bash
mkdir -p frontend/src/components/visual-editor/ComponentRegistry/components/color-picker
```

### 2. 定义组件配置 (config.ts)

```typescript
/**
 * ColorPicker 组件 - 配置
 */

import { ComponentMeta } from '../../types';

// 组件元数据
export const meta: ComponentMeta = {
    type: 'color-picker',        // 唯一类型标识
    name: '颜色选择器',           // 显示名称
    description: '选择颜色值',    // 描述
    icon: 'palette',             // Lucide 图标名
    hasOptions: false,           // 是否有选项列表
};

// 组件定义类型（需先在 types.ts 中定义）
export interface ColorPickerComponentDefinition {
    type: 'color-picker';
    id: string;
    label: string;
    description?: string;
    defaultColor?: string;
    presetColors?: string[];
}

// 创建默认组件定义
export function createDefault(id: string): ColorPickerComponentDefinition {
    return {
        type: 'color-picker',
        id,
        label: '新颜色选择',
        defaultColor: '#6366f1',
        presetColors: ['#ef4444', '#f59e0b', '#10b981', '#3b82f6', '#8b5cf6'],
    };
}
```

### 3. 实现数据块控件 (Control.tsx)

```typescript
/**
 * ColorPicker 组件 - 数据块控件
 * 
 * 在数据块表单中渲染的控件
 */

import { useState } from 'react';
import { ControlProps } from '../../types';
import type { ColorPickerComponentDefinition } from './config';

export function Control({ component, value, onChange, disabled }: ControlProps) {
    const colorDef = component as ColorPickerComponentDefinition;
    const currentColor = (value as string) || colorDef.defaultColor || '#000000';

    return (
        <div className="flex items-center gap-2">
            <input
                type="color"
                value={currentColor}
                onChange={(e) => onChange(e.target.value)}
                disabled={disabled}
                className="w-10 h-10 rounded border cursor-pointer"
            />
            <span className="text-sm font-mono text-slate-600">{currentColor}</span>
            
            {/* 预设颜色 */}
            {colorDef.presetColors && (
                <div className="flex gap-1 ml-2">
                    {colorDef.presetColors.map((color) => (
                        <button
                            key={color}
                            type="button"
                            onClick={() => onChange(color)}
                            disabled={disabled}
                            className="w-6 h-6 rounded border-2 border-white shadow"
                            style={{ backgroundColor: color }}
                        />
                    ))}
                </div>
            )}
        </div>
    );
}

export default Control;
```

### 4. 实现配置表单 (Configurator.tsx)

```typescript
/**
 * ColorPicker 组件 - 配置表单
 * 
 * 在组件配置弹窗中渲染的表单
 */

import { ConfiguratorProps } from '../../types';
import type { ColorPickerComponentDefinition } from './config';

export function Configurator({ formData, errors, onUpdateFormData }: ConfiguratorProps) {
    const colorData = formData as ColorPickerComponentDefinition;

    return (
        <div className="space-y-4">
            {/* 默认颜色 */}
            <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">
                    默认颜色
                </label>
                <input
                    type="color"
                    value={colorData.defaultColor || '#000000'}
                    onChange={(e) =>
                        onUpdateFormData((prev) => ({
                            ...prev,
                            defaultColor: e.target.value,
                        } as ColorPickerComponentDefinition))
                    }
                    className="w-full h-10 rounded border cursor-pointer"
                />
            </div>

            {/* 预设颜色编辑 */}
            <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">
                    预设颜色（逗号分隔）
                </label>
                <input
                    type="text"
                    value={(colorData.presetColors || []).join(', ')}
                    onChange={(e) =>
                        onUpdateFormData((prev) => ({
                            ...prev,
                            presetColors: e.target.value.split(',').map(c => c.trim()).filter(Boolean),
                        } as ColorPickerComponentDefinition))
                    }
                    placeholder="#ef4444, #f59e0b, #10b981"
                    className="w-full px-3 py-2 border rounded-lg text-sm"
                />
            </div>
        </div>
    );
}

export default Configurator;
```

### 5. 创建注册入口 (index.ts)

```typescript
/**
 * ColorPicker 组件 - 注册入口
 * 
 * ⚠️ 必须默认导出 RegisteredComponent 对象
 */

import { RegisteredComponent } from '../../types';
import { meta, createDefault } from './config';
import { Control } from './Control';
import { Configurator } from './Configurator';

const ColorPickerComponent: RegisteredComponent = {
    meta,
    createDefault,
    Control,
    Configurator,
};

export default ColorPickerComponent;
```

### 6. 更新类型定义 (types.ts)

在 `ComponentRegistry/types.ts` 中添加新类型：

```typescript
// 在 ComponentType 中添加
export type ComponentType =
    | 'select'
    | 'multi-select'
    | 'radio'
    | 'checkbox'
    | 'rating'
    | 'number'
    | 'date'
    | 'text'
    | 'textarea'
    | 'file'
    | 'files'
    | 'image'
    | 'images'
    | 'phone'
    | 'email'
    | 'id-card'
    | 'toggle'
    | 'folder-picker'
    | 'avatar'
    | 'file-list'
    | 'tags'
    | 'id-generator'
    | 'color-picker';  // 新增

// 添加组件定义接口
export interface ColorPickerComponentDefinition extends BaseComponentDefinition {
    type: 'color-picker';
    defaultColor?: string;
    presetColors?: string[];
}

// 在联合类型中添加
export type DocumentComponentDefinition =
    | SelectComponentDefinition
    | RatingComponentDefinition
    | NumberComponentDefinition
    | DateComponentDefinition
    | TextComponentDefinition
    | TextareaComponentDefinition
    | FileComponentDefinition
    | FilesComponentDefinition
    | ImageComponentDefinition
    | ImagesComponentDefinition
    | PhoneComponentDefinition
    | EmailComponentDefinition
    | IdCardComponentDefinition
    | ToggleComponentDefinition
    | FolderPickerComponentDefinition
    | AvatarComponentDefinition
    | FileListComponentDefinition
    | TagsComponentDefinition
    | IdGeneratorComponentDefinition
    | ColorPickerComponentDefinition;  // 新增
```

### 7. 完成！

保存文件后，组件会自动被注册。在控制台可以看到：

```
[ComponentRegistry] Registered: color-picker
```

## 核心类型定义

### ComponentMeta（组件元数据）

```typescript
interface ComponentMeta {
    type: ComponentType;      // 唯一类型标识
    name: string;            // 显示名称
    description: string;     // 描述
    icon: string;           // Lucide 图标名
    hasOptions: boolean;    // 是否有选项列表（如下拉菜单）
}
```

### ControlProps（控件 Props）

```typescript
interface ControlProps {
    component: DocumentComponentDefinition;  // 组件定义
    value: string | string[] | number | null | undefined;  // 当前值
    onChange: (value: string | string[] | number | null) => void;  // 值变更回调
    disabled?: boolean;  // 是否禁用
}
```

### ConfiguratorProps（配置器 Props）

```typescript
interface ConfiguratorProps {
    formData: DocumentComponentDefinition;  // 组件定义
    errors: Record<string, string>;  // 错误信息
    onUpdateFormData: (updater: (prev: DocumentComponentDefinition) => DocumentComponentDefinition) => void;
}
```

### RegisteredComponent（注册组件）

```typescript
interface RegisteredComponent {
    meta: ComponentMeta;
    createDefault: (id: string) => DocumentComponentDefinition;
    Control: React.ComponentType<ControlProps>;
    Configurator: React.ComponentType<ConfiguratorProps>;
}
```

## 自动注册机制

框架使用 Vite 的 `import.meta.glob` 实现自动扫描：

```typescript
// registry.ts
const componentModules = import.meta.glob<{ default: RegisteredComponent }>(
    './components/*/index.ts',
    { eager: true }
);

for (const [path, module] of Object.entries(componentModules)) {
    const component = module.default;
    if (component && component.meta && component.meta.type) {
        registry.set(component.meta.type, component);
    }
}
```

**关键点**：
- 扫描 `components/*/index.ts` 模式
- 必须默认导出 `RegisteredComponent` 对象
- 使用 `eager: true` 同步加载

## API 使用

### 获取组件

```typescript
import { 
    getComponent,
    getComponentMeta,
    getControl,
    getConfigurator,
    createDefaultComponent,
    isRegistered,
} from '@/components/visual-editor/ComponentRegistry';

// 获取完整注册信息
const selectComponent = getComponent('select');

// 获取元数据
const meta = getComponentMeta('select');

// 获取控件组件
const SelectControl = getControl('select');

// 获取配置器组件
const SelectConfigurator = getConfigurator('select');

// 创建默认组件定义
const newSelect = createDefaultComponent('select', 'comp-123');

// 检查是否已注册
if (isRegistered('color-picker')) {
    // ...
}
```

### 动态注册（运行时）

```typescript
import { registerComponent, unregisterComponent } from '@/components/visual-editor/ComponentRegistry';

// 注册
registerComponent(myCustomComponent);

// 注销
unregisterComponent('my-custom');
```

## 共享工具

### OptionEditor

用于编辑选项列表的可复用组件：

```typescript
import { OptionEditor } from '../../shared';

<OptionEditor
    options={selectData.options || []}
    onChange={(options) =>
        onUpdateFormData((prev) => ({ ...prev, options }))
    }
/>
```

### getLucideIcon

动态获取 Lucide 图标：

```typescript
import { getLucideIcon } from '../../shared';

const Icon = getLucideIcon('chevron-down');
if (Icon) {
    return <Icon size={16} />;
}
```

### generateComponentId

生成唯一组件 ID：

```typescript
import { generateComponentId } from '../../shared';

const id = generateComponentId(); // 例如: "comp_abc123"
```

### FallbackControl

当组件未找到时的降级控件：

```typescript
import { FallbackControl } from '../../shared';

<FallbackControl
    componentId={componentId}
    value={value}
    onChange={onChange}
/>
```

## 最佳实践

### 1. 类型安全

始终在 `Control.tsx` 和 `Configurator.tsx` 中进行类型断言：

```typescript
const selectDef = component as SelectComponentDefinition;
```

### 2. 优雅降级

处理可能缺失的配置值：

```typescript
const options = selectDef.options || [];
const max = ratingDef.max ?? 5;
```

### 3. Portal 渲染下拉菜单

使用 `createPortal` 渲染下拉菜单，避免被父容器裁剪：

```typescript
import { createPortal } from 'react-dom';

{isOpen && createPortal(
    <div className="dropdown">...</div>,
    document.body
)}
```

### 4. 点击外部关闭

实现点击外部关闭的标准模式：

```typescript
useEffect(() => {
    if (!isOpen) return;
    const handleClickOutside = (e: MouseEvent) => {
        if (triggerRef.current && !triggerRef.current.contains(e.target as Node)) {
            setIsOpen(false);
        }
    };
    document.addEventListener('mousedown', handleClickOutside);
    return () => document.removeEventListener('mousedown', handleClickOutside);
}, [isOpen]);
```

### 5. 样式一致性

使用项目中的 `cn()` 工具和 Tailwind CSS：

```typescript
import { cn } from '@/lib/utils';

<button className={cn(
    'base-styles',
    isActive && 'active-styles',
    disabled && 'disabled-styles'
)}>
```

## 内置组件列表

| 类型 | 名称 | 描述 | 图标 |
|------|------|------|------|
| `select` | 下拉选择 | 单选下拉菜单 | chevron-down |
| `multi-select` | 多选下拉 | 多选下拉菜单 | list |
| `radio` | 单选按钮 | 单选按钮组 | circle-dot |
| `checkbox` | 复选框 | 复选框组 | check-square |
| `rating` | 评分 | 星级评分 | star |
| `number` | 数字输入 | 数字输入框 | hash |
| `date` | 日期选择 | 日期选择器 | calendar |
| `text` | 单行文本 | 单行文本输入 | type |
| `textarea` | 多行文本 | 多行文本输入 | align-left |
| `file` | 单文件 | 单文件选择 | file |
| `files` | 多文件 | 多文件选择 | files |
| `image` | 单图片 | 单图片选择 | image |
| `images` | 多图片 | 多图片选择 | gallery-horizontal |
| `phone` | 手机号 | 手机号输入（带格式化） | smartphone |
| `email` | 邮箱 | 邮箱输入（带验证） | mail |
| `id-card` | 身份证 | 身份证号输入（带掩码） | credit-card |
| `toggle` | 开关 | 布尔值切换开关 | toggle-left |
| `folder-picker` | 目录选择 | 选择文件系统目录（支持多选） | folder |
| `avatar` | 头像 | 头像上传（带裁剪） | user-circle |
| `file-list` | 文件列表 | 只读文件列表展示 | list |
| `tags` | 标签 | 标签输入（支持自定义） | tags |
| `id-generator` | ID生成器 | 自动生成唯一标识 | hash |

## 故障排查

### 组件未自动注册

1. 检查目录结构是否正确
2. 确保 `index.ts` 默认导出 `RegisteredComponent` 对象
3. 检查控制台是否有错误信息
4. 确认 `meta.type` 已添加到 `ComponentType` 联合类型

### 类型错误

1. 确保在 `types.ts` 中定义了组件定义接口
2. 确保添加到了 `DocumentComponentDefinition` 联合类型
3. 使用正确的类型断言

### 控件不显示

1. 检查 `Control.tsx` 是否正确导出
2. 检查 `ControlProps` 是否正确使用
3. 查看浏览器控制台是否有渲染错误

---

**相关文档**：
- [Phase 3.8 - 可视化文档编辑器](./Phase-3.8-可视化文档编辑器.md)
- [Phase 3.9 - 文件管理组件](./Phase-3.9-文件管理组件.md)

