---
slug: doc-ghbnm4
---
# 4.2.12 - 用户ID预生成 API

> 优先级: 🔴 高  
> 预估工期: 1 天  
> 依赖任务: 4.2.4  
> 状态: ✅ 已完成

## 任务概述

实现用户ID预生成 API，前端在创建 user-auth 组件时请求后端预生成唯一 ID。

## 目标

- [x] 实现 ID 生成逻辑
- [x] 单个 ID 生成 API
- [x] 批量 ID 生成 API
- [x] 唯一性保证

## 技术要点

### ID 格式

```
U + 日期 + 序号
例如: U20260106001, U20260106002
```

### 生成逻辑

```typescript
function generateUserId(): string {
  const date = new Date().toISOString().slice(0, 10).replace(/-/g, '');
  const sequence = getNextSequence(date);  // 从索引或计数器获取
  return `U${date}${sequence.toString().padStart(3, '0')}`;
}
```

### API 设计

| 方法 | 路径 | 说明 |
|------|------|------|
| GET | /api/auth/generate-user-id | 生成单个 ID |
| POST | /api/auth/generate-user-ids | 批量生成 ID |

### 请求示例

```typescript
// 单个生成
GET /api/auth/generate-user-id
Response: { "user_id": "U20260106001" }

// 批量生成
POST /api/auth/generate-user-ids
Request: { "count": 5 }
Response: { "user_ids": ["U20260106001", "U20260106002", ...] }
```

## 涉及文件

| 文件 | 修改类型 |
|------|----------|
| `backend/src/services/auth-credential-indexer.ts` | 修改 |
| `backend/src/api/auth.ts` | 修改 |

## 验收标准

1. ID 格式正确
2. ID 保证唯一
3. 批量生成正常工作
4. 前端正确调用

## 进度记录

| 日期 | 进度 | 备注 |
|------|------|------|
| 2026-01-06 | 100% | 全部完成 |

---

*创建日期: 2026-01-06*  
*完成日期: 2026-01-06*

