---
slug: doc-9vjxlu
---
# 4.2.4 - 认证索引服务（严格标识符）

> 优先级: 🔴 高  
> 预估工期: 3 天  
> 依赖任务: 4.2.1  
> 状态: ✅ 已完成

## 任务概述

实现用户认证索引服务，基于 `type: __atlas_user_auth__` 严格标识符识别用户认证组件，构建 `auth-users.json` 索引。

## 目标

- [x] 实现索引数据结构
- [x] 实现文档扫描识别
- [x] 实现凭证映射
- [x] 支持单文档多用户
- [x] 实现索引重建功能

## 技术要点

### 严格标识符

```typescript
const AUTH_BLOCK_TYPE = '__atlas_user_auth__';
```

只识别 `type: __atlas_user_auth__` 的数据块，忽略所有散落字段。

### 索引数据结构

```json
{
  "_meta": {
    "index_name": "auth-users",
    "version": "2.0",
    "identifier": "__atlas_user_auth__",
    "last_full_scan": "...",
    "last_update": "...",
    "stats": { ... }
  },
  "credentials": {
    "zhangsan": { "type": "username", "user_id": "U20260106001" },
    "zhangsan@example.com": { "type": "email", "user_id": "U20260106001" }
  },
  "users": {
    "U20260106001": { ... }
  }
}
```

### 核心函数

```typescript
// 扫描文档，提取所有用户认证组件
function scanDocument(docPath: string, content: string): UserRecord[]

// 重建索引
async function rebuildAuthUsersIndex(): Promise<AuthUsersIndex>

// 获取索引
async function getAuthUsersIndex(): Promise<AuthUsersIndex>
```

## 涉及文件

| 文件 | 修改类型 |
|------|----------|
| `backend/src/services/auth-credential-indexer.ts` | 新增 |
| `backend/src/api/indexes.ts` | 修改 |
| `.atlas/indexes/auth-users.json` | 自动生成 |

## 子任务

- [x] Day 1: 设计索引结构，实现文档扫描
- [x] Day 2: 实现凭证映射，支持单文档多用户
- [x] Day 3: 实现重建 API，测试

## 验收标准

1. 只识别 `__atlas_user_auth__` 数据块
2. 正确构建凭证映射
3. 支持单文档多用户
4. 索引重建 API 正常工作
5. 忽略散落的 email/phone/password 字段

## 进度记录

| 日期 | 进度 | 备注 |
|------|------|------|
| 2026-01-06 | 100% | 全部完成 |

---

*创建日期: 2026-01-06*  
*完成日期: 2026-01-06*

