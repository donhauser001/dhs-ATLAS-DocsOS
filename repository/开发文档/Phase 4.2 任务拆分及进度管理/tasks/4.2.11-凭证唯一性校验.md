# 4.2.11 - 凭证唯一性校验

> 优先级: 🔴 高  
> 预估工期: 1 天  
> 依赖任务: 4.2.4  
> 状态: ✅ 已完成

## 任务概述

实现凭证（用户名、邮箱、手机）唯一性校验，确保在文档保存时验证凭证不与其他用户冲突。

## 目标

- [x] 实现唯一性校验函数
- [x] 文档保存时触发校验
- [x] 前端实时校验反馈
- [x] 错误提示

## 技术要点

### 校验逻辑

```typescript
async function validateCredentialUniqueness(
  docPath: string,
  authData: AuthBlockData
): Promise<{ valid: boolean; error?: string }> {
  const index = await getAuthUsersIndex();
  
  // 检查用户名
  if (authData.username) {
    const existing = index.credentials[authData.username];
    if (existing && existing.user_id !== authData.user_id) {
      return { valid: false, error: `用户名 "${authData.username}" 已被使用` };
    }
  }
  
  // 检查邮箱...
  // 检查手机...
  
  return { valid: true };
}
```

### API 设计

| 方法 | 路径 | 说明 |
|------|------|------|
| POST | /api/auth/validate-credential | 校验凭证唯一性 |

## 涉及文件

| 文件 | 修改类型 |
|------|----------|
| `backend/src/services/auth-credential-indexer.ts` | 修改 |
| `backend/src/api/auth.ts` | 修改 |
| `frontend/src/components/visual-editor/ComponentRegistry/components/user-auth/Control.tsx` | 修改 |

## 验收标准

1. 重复用户名被正确拒绝
2. 重复邮箱被正确拒绝
3. 重复手机被正确拒绝
4. 同一用户修改自己的凭证不被拒绝
5. 前端实时反馈

## 进度记录

| 日期 | 进度 | 备注 |
|------|------|------|
| 2026-01-06 | 100% | 全部完成 |

---

*创建日期: 2026-01-06*  
*完成日期: 2026-01-06*

