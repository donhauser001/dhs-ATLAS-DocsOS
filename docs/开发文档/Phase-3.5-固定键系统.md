...
| 方案 | 优点 | 缺点 | 推荐度 |
|------|------|------|--------|
| **Milkdown** | 插件化、类 Notion、支持协作 | 学习曲线 | ⭐⭐⭐⭐⭐ |
| **BlockNote** | 开箱即用、类 Notion | 定制性略差 | ⭐⭐⭐⭐ |
| **CodeMirror 6** | 性能好、扩展性强 | 偏代码编辑器风格 | ⭐⭐⭐ |
| **Monaco** | 功能强大、VS Code 体验 | 体积大、偏 IDE | ⭐⭐ |
| **Tiptap** | 灵活、社区活跃 | 需要更多配置 | ⭐⭐⭐⭐ |

**推荐方案**：Milkdown 或 BlockNote

```typescript
// 技术栈组合
{
  editor: 'milkdown',           // 主编辑器
...
### Phase 3.5.4: 智能 MD 编辑器 (4-5天)

| 任务 | 说明 | 优先级 |
|------|------|--------|
| 编辑器技术选型 | 评估 Milkdown/BlockNote/Tiptap | P0 |
| SmartEditor 主组件 | 三栏布局框架 | P0 |
| MarkdownPane | MD 编辑区，支持 YAML 高亮 | P0 |
| FieldSettingsPane | 字段设置面板（分区显示） | P0 |
| 双向同步机制 | MD ↔ 表单实时同步 | P0 |
...
| 阶段 | 预计时间 |
|------|---------|
| Phase 3.5.1 固定键定义 | 1-2 天 |
| Phase 3.5.2 自动补齐 | 2-3 天 |
| Phase 3.5.3 渲染分区 | 2-3 天 |
| Phase 3.5.4 智能编辑器 | 4-5 天 |
| Phase 3.5.5 标签扩展 | 1 天 |
| Phase 3.5.6 测试文档 | 1-2 天 |
| **总计** | **11-16 天** |
...
| ID 生成器 | 中文拼音支持 |
| display.json | 渲染分区配置 |
| ZonedBlockRenderer | 分区渲染组件 |
| FooterZone | 可折叠的元数据区 |
| **SmartEditor** | 类 Notion 的智能编辑器 |
| **FieldSettingsPane** | 字段设置面板（右侧栏） |
| **双向同步机制** | MD ↔ 表单实时同步 |
