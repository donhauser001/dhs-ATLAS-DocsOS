...
import { PropertyViewNode, setPropertyViewContext, PropertySelector } from './RichTextEditor/extensions/PropertyView';
...
export function VisualDocEditor({
  ...
  const [showComponentInserter, setShowComponentInserter] = useState(true);
  ...
  // 插入属性引用
  const handleInsertProperty = useCallback((key: string) => {
    if (!editor) return;
  ...
  // 插入组件
  const handleInsertComponent = useCallback((componentType: string, config?: Record<string, unknown>) => {
    if (!editor) return;
    
    // 插入组件占位符或模板
    // 可以插入为特殊的节点，或者插入 YAML 代码块
    const componentTemplate = `\n\`\`\`yaml:component\ntype: ${componentType}\nkey: ${componentType}_${Date.now()}\nlabel: "${componentType}"\n\`\`\`\n`;
    
    editor.chain().focus().insertContent(componentTemplate).run();
  }, [editor]);
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        {viewMode === 'edit' && (
          <ComponentInserter
            ...
            collapsed={!showComponentInserter}
            onToggleCollapse={() => setShowComponentInserter(prev => !prev)}
          />
        )}
      ...
      <div className="sticky bottom-0 z-10 flex items-center justify-between px-6 py-2 border-t border-slate-200 
        ...
        <div className="flex items-center gap-3">
          ...
          {viewMode === 'edit' && (
            <button
              type="button"
              onClick={() => setShowComponentInserter(prev => !prev)}
              className={cn(
                "flex items-center gap-1 px-2 py-1 rounded transition-colors",
                showComponentInserter 
                  ? "text-purple-600 bg-purple-50 hover:bg-purple-100" 
                  : "text-slate-400 hover:text-slate-600 hover:bg-slate-100"
              )}
              title={showComponentInserter ? "隐藏组件面板" : "显示组件面板"}
            >
              {showComponentInserter ? (
