...
const TOKEN_EXPIRY_HOURS = 24; // Token 有效期 24 小时
const TOKENS_FILE = 'activation-tokens.json';
...
function loadTokens(): ActivationTokenStore {
...
function saveTokens(store: ActivationTokenStore): void {
  const filePath = getTokensFilePath();
...
function cleanExpiredTokens(store: ActivationTokenStore): void {
  const now = new Date();
 ...
 * 创建账户激活 Token
 */
export function createActivationToken(userId: string, email: string): string {
 ...
 * 验证激活 Token
 */
export function verifyActivationToken(token: string): { valid: boolean; userId?: string; email?: string } {
  const store = loadTokens();
 ...
 * 使用 Token（标记为已使用）
 */
export function consumeActivationToken(token: string): { success: boolean; userId?: string } {
  const store = loadTokens();
